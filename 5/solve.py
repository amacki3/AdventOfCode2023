                    ...
                    if seed.start <= route.end:
                        if route.start <= seed.end:
                            ...
                            if seed.start < route.start:
                                ...
                                new_seeds.append(seed_range(seed.start, route.start - seed.start))
                                #Alter seed to remove this part we have no dealt with...
                                seed = seed_range(route.start,seed.range - (route.start - seed.start))
                            #Now deal with remaining part within the routes range.
                            if seed.end <= route.end:
                                #careful to use DESTINATION
                                new_start = (seed.start - route.start) + route.destination
                                new_seeds.append(seed_range(new_start,seed.range))
                                seed_exhausted = True
                            else:
                                new_start = (seed.start - route.start) + route.destination
                                new_seeds.append(seed_range(new_start,route.range_value))
                                seed = seed_range(route.end+1,seed.end - route.end)
            if (not seed_exhausted) and (seed.range > 0):
                 new_seeds.append(seed)

        seeds = new_seeds
        seeds = [s for s in seeds if not s.start == 0] # NEEDED TO QUASH BUG?!?!?!
